...
/// Design constants
class _PortalDesign {
  static const neutralBackground = Color(0xFF0F172A);
...
/// Shader-based Portal Page - Apple-grade premium glass effects
class ShaderPortalPage extends HookConsumerWidget {
  const ShaderPortalPage({super.key});
  ...
  Future<void> _triggerMorph(
    ...
    debugPrint('[ShaderPortal] Starting morph for: ${brand.name}');
    ...
    debugPrint('[ShaderPortal] Morph complete');
...
/// Portal content with shader monolith and clean buttons
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        ...
        // Ultra-clean crystal glass monolith with shader
        Hero(
          tag: 'brand-portal',
          child: ShaderGlassCard(
            brand: brand,
...
/// Premium button with mechanical haptic feedback - Sapphire Architect Edition
class _PremiumButton extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTapDown: (_) {
        HapticFeedback.lightImpact(); // Device haptic feedback
        scaleController.reverse(); // Scale down to 0.95
      },
      onTapUp: (_) {
        scaleController.forward(); // Scale back to 1.0
        onTap();
      },
...
// Scanner and Search views remain the same as before
